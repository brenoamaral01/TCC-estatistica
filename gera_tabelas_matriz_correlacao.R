...
# correlações ----
modelos <- list(
  ajust1 = c('log_PIB', 'log_ORC_TOTAL_EMPENHADO', 'DISCR_TX', 'log_SERVIDORES'

  ),
  ajust2 = c('log_PIB', 'ORC_TOTAL_EMPENHADO', 'log_TX_SUCESSO', 'log_DISCR_TX', 'log_INDICE_MNST'
  )
)

# Função para ajustar modelo
ajustar_modelo <- function(nome_modelo, covariaveis, dados) {
  formula_text <- paste("Surv(time = t, event = ADOPT) ~", 
                        paste(covariaveis, collapse = " + "))
  formula_modelo <- as.formula(formula_text)
  
  survreg(formula = formula_modelo, data = dados, dist = 'weibull')
}

# Ajustar os modelos
ajust1 <- ajustar_modelo("ajust1", modelos$ajust1, df_pnab)
ajust2 <- ajustar_modelo("ajust2", modelos$ajust2, df_pse)

# Extrair sumários
summary(ajust1); c(AIC(ajust1), AICc(ajust1), BIC(ajust1))
summary(ajust2); c(AIC(ajust2), AICc(ajust2), BIC(ajust2))

# CORRELAÇÃO - Usando as mesmas definições
library(corrplot)


# Matriz de correlação

# Criar matriz com nomes indicando origem
covariaveis_pnab <- df_pnab[, modelos$ajust1]
covariaveis_pse <- df_pse[, modelos$ajust2]

# Adicionar prefixos aos nomes das colunas
colnames(covariaveis_pnab) <- paste0("PNAB_", colnames(covariaveis_pnab))
colnames(covariaveis_pse) <- paste0("PSE_", colnames(covariaveis_pse))

# Combinar e calcular correlação
matriz_combinada <- cbind(covariaveis_pnab, covariaveis_pse) #, t1, t2)
(matriz_cor <- cor(matriz_combinada, use = "complete.obs"))
#View(matriz_cor)
#matriz_cor <- matriz_cor[!grepl("PNAB", rownames(matriz_cor)), !grepl("PSE", colnames(matriz_cor))]

max(matriz_cor[matriz_cor<1 & !is.na(matriz_cor)])
max(matriz_cor[matriz_cor<.847])

posicoes <- which(matriz_cor>.38, arr.ind = TRUE)
print(paste("Posição do valor", valor_procurado, ":"))
print(posicoes)
...
corrplot(df_pnab, 
         ...
         addCoef.col = "black",
         number.cex = 0.7,
         title = "Correlação entre Covariáveis dos Modelos")


matriz_cor <- cor(df_pnab[,3:18], use = "complete.obs")
library(corrplot)

# 3. Criar o gráfico básico
corrplot(matriz_cor)

# Versão com cores diferentes e números
corrplot(matriz_cor, 
         method = "color",        # Usar cores
         type = "upper",          # Mostrar apenas triângulo superior
         order = "hclust",        # Ordenar por cluster
         addCoef.col = "black",   # Adicionar coeficientes em preto
         tl.col = 'black',
         tl.cex = 0.8,           # Tamanho do texto dos eixos
         number.cex = 0.7) 
#ggsave("imagens/teste_matriz.pdf", width = 205, height = 121, units = "mm")

# PDF com engine Cairo (melhor para fontes e gráficos vetoriais)
# cairo_pdf("imagens/matriz_correlacao_teste.pdf", 
#           width = 12, 
#           height = 10)
...
# pdf("imagens/matriz_correlacao_ajustada.pdf", 
#     width = 10, 
#     height = 8)
...
corrplot(matriz_cor, 
         ...
         tl.cex = 0.8,
         number.cex = 0.7,
         mar = c(0, 0, 2, 0))  # ★ Ajuste crucial aqui
...
tabela_cor <- as.data.frame(as.table(matriz_cor)) %>%
  rename(Var1 = 1, Var2 = 2, Correlacao = 3) %>%
  # Filtrar: correlações fortes e evitar duplicatas (triângulo superior)
  filter(
    abs(Correlacao) > 0.7,    # Correlação forte
    as.character(Var1) < as.character(Var2)  # Evitar duplicatas A-B / B-A
  ) %>%
  # Ordenar por magnitude da correlação (decrescente)
  arrange(desc(abs(Correlacao))) %>%
